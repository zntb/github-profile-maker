/**
 * GitHub Octicon SVG path data and helper for rendering 16×16 stat icons.
 * Moved from app/api/stats/route.ts for potential reuse by other routes.
 */

export type IconEntry = { d: string; fillRule?: 'nonzero' | 'evenodd' };

export const STAT_ICON_PATHS: Record<string, IconEntry[]> = {
  star: [
    {
...
/**
 * Render a 16×16 GitHub Octicon SVG from path data.
 *
 * @param name  – Icon key (e.g. "star", "commit")
 * @param x     – SVG x coordinate
 * @param y     – SVG y coordinate
 * @param color – Hex colour (without #)
 */
export function makeStatIcon(name: string, x: number, y: number, color: string): string {
